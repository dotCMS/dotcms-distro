System.register(['@angular/core', 'rxjs/Rx', "../persistence/ApiRoot", "@angular/http", "./Rule", "../system/http-response-util", "../services/logger.service"], function(exports_1, context_1) {
    "use strict";
    ...
    var core_1, Rx_1, ApiRoot_1, http_1, Rule_1, http_response_util_1, logger_service_1;
    ...
    return {
        ...
        execute: function() {
            ...
            ActionService = (function () {
                function ActionService(apiRoot, http, loggerService) {
                ...
                ActionService.prototype.makeRequest = function (childPath) {
                    ...
                    return this._http.get(path, opts).map(function (res) {
                        ...
                        if (err && err.status === 404) {
                            _this.loggerService.error("Could not retrieve " + _this._typeName + " : 404 path not valid.", path);
                        }
                        else if (err) {
                            _this.loggerService.debug("Could not retrieve" + _this._typeName + ": Response status code: ", err.status, 'error:', err, path);
                        }
                    ...
                    return function (response, original) {
                        if (response) {
                            if (response.status === 500) {
                                ...
                                _this.loggerService.error("Could not execute request: 404 path not valid.");
                                throw new http_response_util_1.CwError(http_response_util_1.UNKNOWN_RESPONSE_ERROR, response.headers.get('error-message'));
                            }
                            else {
                                _this.loggerService.debug("Could not execute request: Response status code: ", response.status, 'error:', response);
                                throw new http_response_util_1.CwError(http_response_util_1.UNKNOWN_RESPONSE_ERROR, response.headers.get('error-message'));
                            }
                        }
                        return null;
